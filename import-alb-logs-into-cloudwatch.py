...
def upload_to_cloud_watch_logs(log_group, log_stream, local_file_name):
    ...
    print("Uploaading " + str(len(log_events)) + " log events")
...
print("S3 Key Prefix ... " + config.s3_key_prefix)
print("Start After ..... " + config.start_after)
...
if contents is None:
    ...
    for item in contents:
        if (config.file_limit >= 0 and file_number >= config.file_limit):
            break
        ...
        local_file_name = config.data_path + "/" + file_name
        ...
        # now upload the data into Cloud Watch Logs
        upload_to_cloud_watch_logs(config.log_group, config.log_stream_name, local_file_name)
